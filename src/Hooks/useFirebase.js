...
const useFirebase = () => {
    ...
    const registerUser = (email, password, name, history) => {
        ...
        createUserWithEmailAndPassword(auth, email, password)
            .then((userCredential) => {
                ...
                const saveUser = {email:email}
                axios.put("https://intense-crag-53623.herokuapp.com/users", saveUser)
                    .then()
    ...
    const googleSignIn = (location, history) => {
        ...
        signInWithPopup(auth, googleProvider)
            .then((result) => {
                ...
                const saveUser = { email: newUser.email};
                axios.put("https://intense-crag-53623.herokuapp.com/users", saveUser)
                    .then()
            }).catch((error) => {
                const errorMessage = error.message;
    ...
    useEffect(() => {
        fetch(`https://intense-crag-53623.herokuapp.com/admin?email=${user.email}`)
            .then(res => res.json())
            .then(data => {
    ...
    useEffect(() => {
        fetch(`https://intense-crag-53623.herokuapp.com/users/${user.email}`)
            .then(res => res.json())
